...
export class PackagesService implements OnModuleInit {
  ...
  async seedDefaults() {
    // Seed CTV
    const ctv = await this.packagesRepository.findOne({
      where: { code: 'CTV' },
    });
    if (!ctv) {
    ...
    const oldNpp = await this.packagesRepository.findOne({
      where: { code: 'NPP', level: 2 },
    });
    if (oldNpp) {
      ...
      await this.dataSource.query(
        `UPDATE users SET packageType = 'TV' WHERE packageType = 'NPP'`,
      );
    }
    ...
    if (!tv) {
      ...
      // Let's safe check:
      const tvCheck = await this.packagesRepository.findOne({
        where: { code: 'TV' },
      });
      if (!tvCheck) {
  ...
  async update(
    id: string,
    updatePackageDto: Partial<Package>,
